# fxapi_client.py
# Minimal robust FXAPI wrapper with retries, idempotency and small helpers used by manager.
...
BASE = "https://fxapi.io"  # using earlier example endpoints: adapt if provider uses different paths
# Note: some fxapi providers use different endpoints. If yours requires /order or different fields, adapt here.

def _headers():
    return {"Authorization": f"Bearer {FXAPI_TOKEN}", "Content-Type": "application/json"}

def _retry_post(path, json_payload, timeout=2):
...
def _retry_get(path, params=None):
    url = f"{BASE}{path}?token={FXAPI_TOKEN}"
    backoff = 0.05
    for attempt in range(MAX_RETRIES):
        try:
